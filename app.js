  ...
  return `<div>
    ...
    <main class="container">
      <section class="hero">
        ...
        <div class="hero-art"><div class="hero-badge"><span>Jump Metrics</span><b>A avaliação inteira em uma sequência simples.</b><span>Cadastre, meça, escaneie, anexe e finalize.</span></div></div>
      </section>
...
function wizardStepContent(){
  const w=state.wizard,d=w.data;
  if(w.step===1)return `<div class="measure-grid">${field('assessed_at','Data da avaliação','date')}${field('weight_kg','Peso (kg)')}${field('height_cm','Altura (cm)')}</div><div class="ratio-grid"><div class="ratio"><span>IMC calculado</span><b>${num(d.weight_kg)&&num(d.height_cm)?(num(d.weight_kg)/(num(d.height_cm)/100)**2).toFixed(1):'—'}</b></div></div>`;
  ...
  if(w.step===3)return `<div class="circ-grid">${circFields.map(([k,l])=>field(k,`${l} (cm)`)).join('')}</div><div class="ratio-grid"><div class="ratio"><span>Relação cintura/quadril</span><b>${ratio('waist_cm','hip_cm')}</b></div><div class="ratio"><span>Relação ombro/cintura</span><b>${ratio('shoulder_cm','waist_cm')}</b></div><div class="ratio"><span>Relação cintura/altura</span><b>${num(d.waist_cm)&&num(d.height_cm)?(num(d.waist_cm)/num(d.height_cm)).toFixed(2):'—'}</b></div></div>`;
...
function bindGlobal(){
  ...
  document.querySelectorAll('.wizard-input').forEach(el=>el.oninput=()=>{state.wizard.data[el.name]=el.value;if(state.wizard.step===1||state.wizard.step===3)render()});
